...
typedef struct __attribute__((__packed__)) byte_ {
    uint8_t nibble1: 4;
    uint8_t nibble2: 4;
} byte_;
// define the SBox
inline uint8_t S[] = {0xC, 0x5, 0x6, 0xB, 0x9, 0x0, 0xA, 0xD, 0x3, 0xE, 0xF, 0x8, 0x4, 0x7, 0x1, 0x2};
// define the inverse SBox
inline uint8_t invS[] = {0x5, 0xe, 0xf, 0x8, 0xC, 0x1, 0x2, 0xD, 0xB, 0x4, 0x6, 0x3, 0x0, 0x7, 0x9, 0xA};
// define the permutation table
inline uint8_t P[] = {0, 16, 32, 48, 1, 17, 33, 49, 2, 18, 34, 50, 3, 19, 35, 51,
                      4, 20, 36, 52, 5, 21, 37, 53, 6, 22, 38, 54, 7, 23, 39, 55,
                      8, 24, 40, 56, 9, 25, 41, 57, 10, 26, 42, 58, 11, 27, 43, 59,
                      12, 28, 44, 60, 13, 29, 45, 61, 14, 30, 46, 62, 15, 31, 47, 63};

// function for converting a Hex String to an array of bytes
inline byte_ *fromHexStringToBytes(char *block) {
    byte_ *bytes = (byte_ *) malloc(8 * sizeof(byte_));
    int i;
    // each character of the String is one nibble (4 bits), there are 8 bytes in a 64-bit block
    for (i = 0; i < 8; i++) {
        bytes[i].nibble1 = (block[2 * i] >= '0' && block[2 * i] <= '9') ? (block[2 * i] - '0') : (block[2 * i] - 'a' +
                                                                                                  10);
        bytes[i].nibble2 = (block[2 * i + 1] >= '0' && block[2 * i + 1] <= '9') ? (block[2 * i + 1] - '0') : (
                block[2 * i + 1] - 'a' + 10);
    }
    return bytes;
}
...
inline uint64_t fromBytesToLong(byte_ *bytes) {
    uint64_t result = 0;
    int i;
...
inline uint64_t fromHexStringToLong(char *block) {
    uint64_t result;
    int i;
...
inline byte_ *fromLongToBytes(uint64_t block) {
    byte_ *bytes = (byte_ *) malloc(8 * sizeof(byte_));
    int i;
...
inline uint8_t Sbox(uint8_t input) {
    return S[input];
}

// inverse function of the one above (used to obtain the original nibble)
inline uint8_t inverseSbox(uint8_t input) {
    return invS[input];
}
...
*/
inline uint64_t permute(uint64_t source) {
...
*/
inline uint64_t inversepermute(uint64_t source) {
    uint64_t permutation = 0;
    int i;
    for (i = 0; i < 64; i++) {
        int distance = 63 - P[i];
...
inline uint16_t getKeyLow(char *key) {
    int i;
    uint16_t keyLow = 0;
...
inline uint64_t *generateSubkeys(char *key) {
    ...
    uint64_t *subKeys = (uint64_t *) malloc(32 * (sizeof(uint64_t)));
    int i;
...
//    printf("\n");
    ...
    for (i = 0; i < 31; i++) {
        ...
        byte_ *stateBytes = fromLongToBytes(state);
